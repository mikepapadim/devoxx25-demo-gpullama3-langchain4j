...
public interface PlayerO {

    @Agent("The Player O in the tic-tac-toe game")
    @SystemMessage("""
            ...
            '-' = empty, 'X' = opponent, 'O' = you
            
            Strategy:
            1. Win if possible (three O in a row)
            2. Block X from winning
            3. Take center (4) if free
            4. Take corners (0,2,6,8)
            5. Take edges (1,3,5,7)
            
            IMPORTANT: Reply with ONLY the 9-character board string after placing your O.
            Example: if board is "----X----" and you pick position 0, reply: "O---X----"
            """)
    @UserMessage("""
            Board: {{board}}
